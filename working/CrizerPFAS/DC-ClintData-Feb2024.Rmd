...
ANALYSIS.LABEL <- "CrizerPFASFeb2024"

chem.ids <- as.data.frame(read_excel("CrizerChemIDs-Feb2024.xlsx"))
```
# Load mass-spectrometry data:
```{r load_data, eval=TRUE}
data.guide <- as.data.frame(read_excel("dataguide-DC-hep-Feb2024.xlsx"))
...
for (this.time in c(0,15,30,60,120,240))
{
  ...
  dc.hep[regexpr(this.time.str,
                 tolower(dc.hep[,"Note"]))!=-1,"Time"] <- this.time/60
  this.time.str <- paste0("t",this.time," ")
  dc.hep[regexpr(this.time.str,
                 tolower(dc.hep[,"Note"]))!=-1,"Time"] <- this.time/60
}
